...
Chat, vision, and image generation deployments are stored in LiteLLM's PostgreSQL-backed control plane and maintained day to day from the Paply Gateway model page. The native LiteLLM UI is only the advanced diagnostic escape hatch. The desktop continues to use the stable `paply-chat`, `paply-vision`, and `paply-image` aliases. Add multiple deployments with the same alias for load balancing. The checked-in Router strategy is `simple-shuffle`: without capacity metadata it distributes requests randomly; numeric `weight`, RPM, or TPM values configured per deployment refine the split. Refresh node health from the Paply Gateway page before enabling a deployment; the UI evaluates the target deployment inside LiteLLM's `healthy_endpoints` and `unhealthy_endpoints` instead of treating an HTTP 200 from `/health` as proof of health.
