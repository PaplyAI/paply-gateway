...
Chat, vision, and image generation deployments are stored in LiteLLM's PostgreSQL-backed control plane and maintained day to day from the PaplyAI Gateway model page. The native LiteLLM UI is only the advanced diagnostic escape hatch. The desktop continues to use the stable `paply-chat`, `paply-vision`, and `paply-image` aliases. Add multiple deployments with the same alias for load balancing. The checked-in Router strategy is `simple-shuffle`: a new `paply-chat` session may start on any healthy deployment, then LiteLLM session affinity keeps all follow-up requests on that exact deployment for 30 days. This is required even when every deployment supports the Responses API, because native reasoning and tool-call state cannot cross providers or actual models. Vision and image groups remain free to load-balance per request; numeric `weight`, RPM, or TPM values configured per deployment refine their split. Refresh node health from the PaplyAI Gateway page before enabling a deployment; the UI evaluates the target deployment inside LiteLLM's `healthy_endpoints` and `unhealthy_endpoints` instead of treating an HTTP 200 from `/health` as proof of health.

The public edge never trusts a caller-supplied `x-litellm-session-id` or `x-litellm-trace-id`. Desktop sends `x-paply-session-id`; the edge validates it, namespaces it by the authenticated Paply user, HMACs the result, and forwards only the derived `x-litellm-session-id`. Older Pi clients that already send a valid `x-client-request-id` receive the same compatibility behavior. Redis append-only persistence stores LiteLLM's affinity map. If a pinned deployment is unavailable, the Paply LiteLLM wrapper returns no eligible deployment and the request fails visibly; it must never fall back to another provider for that stateful session. Removing the Redis volume or allowing the 30-day TTL to expire permits a later request to choose a different deployment, so do not do either while long-running sessions are active.
